...
export function GuestForm({ index, huesped, onChange, onAutoCompletar }: GuestFormProps) {
    ...
    <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-3">
      <div className="flex justify-between items-center">
        ...
        {estadoEdad && (
          <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-md ${
            estadoEdad.includes('Adulto') ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-amber-50 text-amber-600 border border-amber-100'
          }`}>
            {estadoEdad}
          </span>
        )}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="relative flex flex-col justify-end">
          <span className="text-[8px] font-bold text-slate-400 uppercase ml-1 mb-0.5">F. de Nacimiento</span>
          <input 
            required
            type="date"
