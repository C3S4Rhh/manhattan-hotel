export function ClientCard({ data }: { data: any }) {
  ...
  // Estilos dinámicos
  const borderColor = isFinalizado
  ...
  return (
    <div
      className={`bg-white p-5 rounded-2xl border-l-4 ${borderColor} shadow-lg border border-slate-100 hover:shadow-xl transition-all`}
    >
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <span
          className={`text-[9px] font-black ${badgeBg} ${badgeText} px-2 py-1 rounded-lg uppercase`}
        >
          {data.estado}
        </span>
      </div>
      ...
      <div className="space-y-2 mt-4 pt-4 border-t border-slate-50">
        <div className="grid grid-cols-2 gap-2 text-[10px]">
          <p>
            ...
            {infoCliente.documento || "N/A"}
          </p>
          <p>
            <span className="text-slate-400">EDAD:</span>{" "}
            {calcularEdad(infoCliente.fecha_nacimiento)} años
          </p>
          <p>
            <span className="text-slate-400">NAC:</span>{" "}
            {infoCliente.nacionalidad || "N/A"}
          </p>
          <p>
            <span className="text-slate-400">EST. CIVIL:</span>{" "}
            {infoCliente.estado_civil || "N/A"}
          </p>
          <p>
            <span className="text-slate-400">PROF:</span>{" "}
            {infoCliente.profesion || "N/A"}
          </p>
          <p>
            <span className="text-slate-400">CEL: </span>{" "}
            {infoCliente.celular || "N/A"}
          </p>
        </div>
      </div>
      ...
      {/* Saldo Pendiente */}
      <div
        className={`p-2 rounded-lg text-center ${saldoPendiente > 0 ? "bg-rose-50" : "bg-slate-50"}`}
      >
        <p
          className={`font-bold uppercase text-[9px] ${saldoPendiente > 0 ? "text-rose-400" : "text-slate-400"}`}
        >
          Saldo Pendiente
        </p>
        <p
          className={`font-black text-sm ${saldoPendiente > 0 ? "text-rose-600" : "text-slate-800"}`}
        >
