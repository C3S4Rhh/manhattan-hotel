...
export function PanelCaja({ usuario }: { usuario: any }) {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <p className="font-black italic text-slate-400 animate-pulse tracking-widest uppercase">
      ...
      <div className="min-h-screen bg-slate-50 p-8 flex items-center justify-center">
        <div className="bg-white rounded-[2rem] shadow-2xl border border-slate-100 max-w-md w-full overflow-hidden">
          <div className="bg-[#1e293b] p-8 text-white text-center">
            ...
            <h2 className="text-2xl font-black uppercase tracking-tighter mt-2">
              Caja de Recepción Cerrada
            </h2>
            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-1">
              Aperture el turno para operar el sistema
            </p>
          </div>
          <form
  ...
  const saldoEnCajaTeorico =
    Number(sesionActiva.monto_inicial) +
    ...
    <div className="bg-slate-50 p-4 md:p-8 rounded-3xl shadow-inner min-h-screen space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 max-w-7xl mx-auto">
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between">
          ...
          <span className="text-2xl font-black text-slate-700 mt-2">
            {Number(sesionActiva.monto_inicial).toFixed(2)} Bs.
          </span>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between">
          ...
          <span className="text-2xl font-black text-emerald-600 mt-2">
            +{totalEfectivoIngresos.toFixed(2)} Bs.
          </span>
        </div>
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between">
        ...
        <div className="bg-slate-900 p-6 rounded-2xl shadow-lg text-white flex flex-col justify-between">
          ...
          <span className="text-2xl font-black text-blue-400 mt-2">
            {saldoEnCajaTeorico.toFixed(2)} Bs.
          </span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-slate-100">
        ...
        <div className="bg-[#1e293b] p-8 text-white flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-1">
              Operador actual: {usuario?.nombre || "Cesar"} — Abierta el:{" "}
              ...
              <table className="w-full min-w-[1350px]">
                <thead>
                  <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                    ...
                    <th className="p-4 text-left min-w-[250px]">Observaciones</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {movimientos.length === 0 ? (
                    <tr>
                      <td
                        colSpan={14}
                        className="p-8 text-center text-slate-300 font-bold italic text-sm"
                      >
                        ...
                        >
                          <td className="p-4 text-xs font-medium text-slate-500 whitespace-nowrap">
                            ...
                            <span className="text-[10px] text-slate-400">
                              {new Date(m.fecha).toLocaleTimeString("es-BO", {
                                hour: "2-digit",
                                minute: "2-digit",
                              })}
                            </span>
                          ...
                          <td className="p-4 text-xs font-bold text-slate-600 capitalize whitespace-nowrap">
                          ...
                          <td className="p-4 text-sm font-black text-blue-600 whitespace-nowrap">
                            {numeroDeHabitacion ? `#${numeroDeHabitacion}` : "---"}
                          </td>

                          <td className="p-4 text-sm font-black text-slate-700 whitespace-nowrap">
                            {totalEstadia > 0 ? `${totalEstadia}` : "-"}
                          </td>

                          <td className="p-4 text-right text-sm font-semibold text-slate-600 whitespace-nowrap">
                            {precioUnitario > 0
                              ? `${precioUnitario.toFixed(2)} Bs.`
                              : "0.00 Bs."}
                          </td>

                          <td className="p-4 text-right text-sm font-black text-blue-700 bg-blue-50/30 whitespace-nowrap">
                            {totalAPagar > 0
                              ? `${totalAPagar.toFixed(2)} Bs.`
                              : "0.00 Bs."}
                          </td>

                          <td className="p-4 text-right text-sm font-black text-emerald-600 whitespace-nowrap">
                            {montoEfectivoRow > 0
                              ? `+${montoEfectivoRow.toFixed(2)} Bs.`
                              : "0.00 Bs."}
                          </td>

                          <td className="p-4 text-right text-sm font-black text-indigo-600 whitespace-nowrap">
                            {montoQrRow > 0
                              ? `+${montoQrRow.toFixed(2)} Bs.`
                              : "0.00 Bs."}
                          </td>

                          <td className="p-4 text-right text-sm font-black text-slate-700 bg-slate-50/50 whitespace-nowrap">
                            {deudeRestante > 0
                              ? `${deudeRestante.toFixed(2)} Bs.`
                              : "0.00 Bs."}
                          </td>
                          <td className="p-4 text-right text-sm font-semibold text-slate-600 whitespace-nowrap">
                          ...
                          <td className="p-4 text-xs min-w-[250px] whitespace-normal break-words">
                            {deudeRestante > 0 &&
                            m.tipo_movimiento.toLowerCase() === "ingreso" ? (
                              <span className="inline-block mb-1.5 px-2 py-1 rounded bg-rose-50 text-rose-600 font-bold uppercase text-[9px] border border-rose-100">
                                🔴 DEBE SALDO
                              </span>
                            ) : (
                              <span className="inline-block mb-1.5 px-2 py-1 rounded bg-emerald-50 text-emerald-600 font-bold uppercase text-[9px] border border-emerald-100">
                                🟢 PROCESADO
                              </span>
                            )}

                            {m.observaciones && (
                              <p className="text-[11px] text-slate-600 mt-1 italic font-medium leading-relaxed">
                                {m.observaciones}
                              </p>
                            )}

                            {m.observaciones_hospedaje && (
                              <p className="text-[11px] text-indigo-600 mt-1 italic font-medium leading-relaxed">
                                {m.observaciones_hospedaje}
                              </p>
                            )}
                          </td>
            ...
            >
              <div className="bg-slate-50 p-4 rounded-xl text-center border border-slate-100 space-y-1">
                ...
                <p className="text-[10px] text-indigo-600 font-bold">
                  Total QR en Turno:{" "}
                  {(totalQrIngresos + totalExtrasQr).toFixed(2)} Bs.
                </p>
              </div>
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 bg-slate-100 text-slate-500 font-bold py-3 rounded-xl text-xs uppercase"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={cargandoAccion}
                  className="flex-1 bg-rose-600 hover:bg-rose-700 text-white font-black py-3 rounded-xl text-xs uppercase tracking-wider"
                >
                  {cargandoAccion
                    ? "Cerrando..."
                    : "Confirmar Cierre y Descargar PDF"}
                </button>
              </div>
            </form>
          </div>
