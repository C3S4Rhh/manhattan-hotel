...
const CONFIG_LIMPIEZA_UI: Record<string, { label: string; color: string }> = {
  lo: { label: "L.O.", color: "text-amber-600 bg-amber-50" },
  sucio: { label: "Sucio", color: "text-rose-600 bg-rose-50" },
  "s-12": { label: "S-12", color: "text-purple-600 bg-purple-50" },
  limpio: { label: "Limpio", color: "text-emerald-600 bg-emerald-50" },
  sl: { label: "S.L", color: "text-blue-600 bg-blue-50" },
};

export function DirectorioHabitaciones({
  habitaciones,
  onUpdate,
}: {
  habitaciones: any[];
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-slate-50">
              {habitaciones.map((hab) => {
                console.log(
                  `Habitación ${hab.numero}: estado original es ->`,
                  hab.estado_actual,
                );

                let valorRaw = String(hab.estado_actual || "L").trim();

                const estadoKey =
                  valorRaw === "0" || valorRaw === "O" ? "O" : valorRaw;

                const config = CONFIG_ESTADOS[estadoKey] || CONFIG_ESTADOS["L"];
                const limpiezaRaw = String(
                  hab.estado_limpieza || "limpio",
                ).toLowerCase();
                const configLimpieza = CONFIG_LIMPIEZA_UI[limpiezaRaw] || {
                  label: "Limpio",
                  ...
                  >
                    ...
                    <td className="p-4">
                      <div className="flex items-center gap-4">
                        ...
                        />
                        <div>
                          <p className="font-black text-slate-700 uppercase text-sm leading-none mb-1">
                            {hab.tipo}
                          </p>
                          <div
                            className={`inline-block px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-tighter border ${config.border} ${config.bg} ${config.text}`}
                          >
                            Estado: {config.label}
                          </div>
                          {/* Etiqueta Estado Limpieza NUEVA */}
                          <div
                            className={`px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-tighter border border-current ${configLimpieza.color}`}
                          >
                            {configLimpieza.label}
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="p-4">
                      <div className="flex flex-col">
                        ...
                        <span className="font-black text-slate-600 text-lg">
                          Bs. {hab.precio_base}
                        </span>
                      </div>
                    </td>
                    <td className="p-4 text-center">
                      <button
                        onClick={() => {
                          // Normalizamos el estado antes de pasarlo al modal
                          const estadoRaw = String(
                            hab.estado_actual || "L",
                          ).trim();
                          const estadoNormalizado =
                            estadoRaw === "0" || estadoRaw === "O"
                              ? "O"
                              : estadoRaw;

                          // Creamos un objeto clonado con el estado ya limpio
                          setHabitacionParaEditar({
                            ...hab,
