...
export function CheckInModal({ hab, usuario, onClose, onSuccess }: any) {
  const {
    ...
    actualizarHuesped,
    autoCompletarHuesped,
    registrarIngreso,
  } = useCheckIn(hab, usuario, onSuccess);

  const displayResponsable =
    usuario?.nombre || usuario?.user_metadata?.nombre || "Cesar";
      ...
      <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden border border-slate-100 max-h-[90vh] flex flex-col">
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-black text-sky-600 uppercase ml-1">
                Pago QR (Bs.)
              </label>
              <input
                type="number"
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-1">
              Estado de Limpieza al Ingresar
            </label>
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={() => setEstadoLimpieza("lo")}
                className={`flex-1 py-3 rounded-xl text-[10px] font-black transition-all ${estadoLimpieza === "lo" ? "bg-rose-500 text-white" : "bg-slate-100 text-slate-400"}`}
              >
                L.O.
              </button>
            </div>
          ...
          <div className="space-y-4 border-t border-slate-100 pt-4">
            ...
            {huespedes.map((h, i) => (
              <GuestForm
                ...
                onAutoCompletar={autoCompletarHuesped}
              />
            ))}
          </div>
