...
export function ListaReservas({ onBack }: { onBack: () => void }) {
  ...
  // Función auxiliar para calcular las noches de estadía
  const calcularNoches = (inicio: string, fin: string) => {
    ...
    const dateInicio = new Date(anioIni, mesIni - 1, diaIni);
    const dateFin = new Date(anioFin, mesFin - 1, diaFin);
    const diffTime = dateFin.getTime() - dateInicio.getTime();
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-8">
      ...
      {cargando ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {reservas.length > 0 ? (
            reservas.map((r) => {
              const nochesEstadia = calcularNoches(r.fecha_inicio, r.fecha_fin);
              return (
                <div
                  key={r.id}
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    {/* Reemplazado: Estadia Ini / Fin por Noches de Estadía */}
                    <div>
                      <p className="text-[9px] font-black text-slate-400 uppercase">
                        Noches de Estadía
                      </p>
                      <p className="font-black text-blue-600">
                        {nochesEstadia} {nochesEstadia === 1 ? "noche" : "noches"} <br />
            ...
            <div className="col-span-3 text-center py-10 text-slate-400 font-bold">
